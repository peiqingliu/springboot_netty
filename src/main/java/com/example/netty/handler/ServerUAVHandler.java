...
import com.example.netty.entity.Student;
...
public class ServerUAVHandler extends ChannelInboundHandlerAdapter {
    ...
    public void simpleRead(ChannelHandlerContext ctx, Object msg) throws UnsupportedEncodingException {
        ...
        //讲数组中的数据转成string类型的数据
        String reqStr = new String(reqByte, "UTF-8");*/
        log.info("服务端接收到客户端的数据${reqStr}:" + result);


        String respStr = new StringBuilder("来自服务器的响应:").append("success").toString();
        // 返回给客户端响应
        ByteBuf resp = Unpooled.copiedBuffer(respStr.getBytes());

        // 返回给客户端响应
        // 当信息返回给客户端后中断, 和客户端链接中断即短连接
